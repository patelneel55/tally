...
from infra.core.exceptions import PipelineError, AcquisitionError
...
from infra.pipelines.rag_pipeline import RAGFinancialAnalysisPipeline # Or your specific RAG pipeline class
...
class QueryOrchestrator:
    ...
    def _needs_indexing(self, source_entity: str) -> bool:
        ...
        """
        max_staleness = self.config.get(f'staleness_days_{source_entity}', self.default_staleness_days)
        ...
        try:
            ...
            exists_and_fresh = self.vector_store.check_source_exists(
                ...
                metadata_field=self.config.get('source_metadata_field', 'source_entity'), # Configurable field name
                max_staleness_days=max_staleness
            )
            logger.info(f"Checked vector store for '{source_entity}'. Exists and fresh: {exists_and_fresh}")
            return not exists_and_fresh
        except Exception as e:
             logger.error(f"Error checking vector store for '{source_entity}': {e}", exc_info=True)
    ...
    def handle_query(self, query: str) -> AnalysisResult: # Or return type of your RAG pipeline
        """
        ...
        if not source_entity:
            logger.warning("No source entity identified, proceeding directly to RAG without indexing check.")
            ...
            if self._needs_indexing(source_entity):
                ...
                try:
                    # 1. Fetch Data
                    fetch_params = self.config.get(f'fetch_params_{source_entity}', {}) # Source-specific fetch params
                    logger.info(f"Fetching data for '{source_entity}' with params: {fetch_params}")
                    # data_uris = self.data_fetcher.fetch(source_entity, params=fetch_params)
                    # ---> Mocking fetch for now
                    data_uris = [f'./fake_data/{source_entity}_report.pdf'] # Replace with actual fetch call
                    logger.info(f"Data URIs obtained: {data_uris}")

                    if not data_uris:
                         logger.warning(f"No data URIs found after fetching for {source_entity}. Skipping indexing.")
                    else:
                        # 2. Run Indexing Pipeline for each fetched resource
                        for uri in data_uris:
                            try:
                                logger.info(f"Running indexing pipeline for URI: {uri}")
                                self.indexing_pipeline.run(uri) # Assumes pipeline handles metadata tagging
                            except PipelineError as e:
                                logger.error(f"Indexing failed for {uri}: {e}", exc_info=True)
                                # Decide: Continue with other URIs? Abort?
                            except Exception as e:
                                 logger.error(f"Unexpected error during indexing for {uri}: {e}", exc_info=True)
                    ...
                    logger.error(f"Data acquisition failed for {source_entity}: {e}", exc_info=True)
                    # Decide: Proceed to RAG anyway? Return error?
                except Exception as e:
                     logger.error(f"Unexpected error during acquisition/indexing phase for {source_entity}: {e}", exc_info=True)
            else:
                logger.info(f"Source '{source_entity}' is already indexed and fresh. Skipping indexing.")

        # 3. Run RAG Pipeline (always runs, potentially with newly indexed data)
        try:
            logger.info("Proceeding to RAG pipeline...")
            result = self.rag_pipeline.run(task_description=query) # Pass the original query
            ...
            logger.error(f"Unexpected error during RAG pipeline execution: {e}", exc_info=True)
            raise
