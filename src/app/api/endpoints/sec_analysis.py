...
async def analyze_latest_filing(
    ...
    filing_type: FilingType = Query(
        FilingType.FORM_10K, description="Type of SEC filing to analyze"
    ),
    limit: int = Query(
        1,
        description="Number of filings to retrieve (only the most recent will be analyzed)",
    ),
):
    """
    ...
    try:
        ...
        filings = await sec_fetcher.get_sec_filings(
            symbol=symbol, filing_type=filing_type, limit=limit
        )
...
async def legacy_sec_filings_redirect(
    request: Request, symbol: str = Path(..., description="Stock symbol of the company")
):
    """
    ...
    logger.info(
        f"Redirecting legacy SEC filings request for {symbol} to {redirect_url}"
    )
