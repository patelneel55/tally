...
from app.models.financial_statements import FilingType, FinancialStatementType
from app.services.excel_exporter import (
    excel_exporter,
    export_financial_statements_to_excel,
)
...
async def export_financials(
    ...
    statements: Optional[List[str]] = Query(
        None,
        description="Statement types to include (income_statement, balance_sheet, cash_flow). If empty, all are included.",
    ),
):
    """
    ...
    """
    logger.info(
        f"Exporting financial statements for {symbol} {fiscal_year} {fiscal_period}"
    )
    ...
    try:
        ...
        if filing_type == "10-K":
            ...
            raise HTTPException(
                status_code=400, detail=f"Unsupported filing type: {filing_type}"
            )
            ...
            for statement_name in statements:
                try:
                    ...
                    if statement_type in all_statements:
                        filtered_statements[statement_type] = all_statements[
                            statement_type
                        ]
                except ValueError:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to export financial statements: {str(e)}"
        )


@router.get(
    "/{symbol}/multi-period",
    summary="Export multi-period financial statements to Excel",
)
async def export_multi_period_financials(
    ...
    """
    logger.info(
        f"Exporting multi-period financial statements for {symbol} (years: {years}, quarterly: {quarterly})"
    )
    ...
    try:
        ...
        for year in reversed(target_years):  # Start with most recent for latest_filing
            try:
                filing = sec_filing_fetcher.get_filing(
                    ticker=symbol, filing_type=filing_type_enum, fiscal_year=year
                )
                ...
                logger.warning(
                    f"Error fetching {filing_type} for {symbol} in {year}: {str(e)}"
                )
        ...
        for statement_type in FinancialStatementType:
            ...
            if combined_metrics:
                ...
                if latest_filing and statement_type in all_statements.get(
                    target_years[-1], {}
                ):
        ...
        logger.error(
            f"Error exporting multi-period financial statements: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to export financial statements: {str(e)}"
