...
async def analyze_sec_filing(
    ...
    filing_date: Optional[str] = Query(
        None, description="Filing date in ISO format (YYYY-MM-DD)"
    ),
):
    """
    ...
    try:
        ...
        filings = await sec_fetcher._query_sec_filings(
            symbol=symbol, form_type=filing_type, limit=1
        )

        if not filings:
            raise HTTPException(
                status_code=404, detail=f"No {filing_type} filings found for {symbol}"
            )
        ...
        if not filing:
            raise HTTPException(
                status_code=404,
                detail=f"Failed to process {filing_type} filing for {symbol}",
            )
...
async def analyze_historical_filings(
    ...
    try:
        ...
        if not trends_analysis:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to analyze historical filings for {symbol}",
            )
...
async def get_historical_filings(
    ...
    try:
        ...
        if not historical_filings or not any(historical_filings.values()):
            raise HTTPException(
                status_code=404, detail=f"No historical filings found for {symbol}"
            )

        # Convert to response format
        response = {"symbol": symbol, "filings": {}}
...
async def get_sec_filings(
    ...
    filing_type: Optional[str] = Query(
        None, description="Type of filing to filter by (e.g., 10-K, 10-Q, 8-K)"
    ),
    limit: Optional[int] = Query(
        20, ge=1, le=100, description="Maximum number of filings to retrieve"
    ),
):
    """
    ...
    """
    logger.info(
        f"Retrieving SEC filings for {symbol}, filing_type={filing_type}, limit={limit}"
    )
    try:
        ...
        filings = await sec_fetcher._query_sec_filings(
            symbol=symbol, form_type=filing_type, limit=limit
        )
        logger.info(
            f"Received response from _query_sec_filings: {len(filings) if filings else 0} filings"
        )
        ...
        if not filings:
            ...
            logger.warning(
                f"No SEC filings found for {symbol}. Debug info: {debug_info}"
            )
            return {
                "symbol": symbol,
                "filings": [],
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving SEC filings: {str(e)}"
        )
...
async def legacy_get_sec_filings(
    ...
    if isinstance(result, dict):
        result["notice"] = (
            "This endpoint is deprecated. Please update your client to use /sec-data/{symbol}/filings instead."
        )
