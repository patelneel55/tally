...
class SECFilingAnalysisResult:
    ...
    @classmethod
    def from_dict(
        cls, data: Dict[str, Any], filing: SECFiling
    ) -> "SECFilingAnalysisResult":
...
class SECFilingAnalyzer:
    ...
    async def analyze_filing(
    ...
    ) -> Optional[SECFilingAnalysisResult]:
        """
        ...
        """
        logger.info(
            f"Analyzing {filing.filing_type} filing for {filing.symbol} from {filing.filing_date}"
        )
        ...
        if cache_path.exists() and settings.ENABLE_CACHE:
            logger.info(
                f"Using cached analysis for {filing.symbol} {filing.filing_type} from {filing.filing_date}"
            )
            try:
        ...
        try:
            ...
            if filing.document_url:
                logger.info(
                    f"Downloading SEC filing PDF for {filing.symbol} {filing.filing_type} from {filing.filing_date}"
                )
                try:
                    ...
                    return SECFilingAnalysisResult(
                        ...
                        analysis={
                            "ERROR": f"Failed to download SEC filing PDF: {str(e)}"
                        },
                    )
            ...
            if not analysis_result:
                logger.error(
                    f"Failed to analyze filing for {filing.symbol} {filing.filing_type}"
                )
                return SECFilingAnalysisResult(
                    filing=filing,
                    summary=f"Error: Failed to analyze SEC filing for {filing.symbol}",
                    analysis={
                        "ERROR": "Analysis process failed. Please check logs for details."
                    },
                )
            ...
            try:
                ...
                logger.info(
                    f"Successfully cached analysis for {filing.symbol} {filing.filing_type}"
                )
            ...
            logger.info(
                f"Successfully analyzed filing for {filing.symbol} {filing.filing_type}"
            )
            return analysis_result

        except Exception as e:
            logger.error(
                f"Error analyzing filing for {filing.symbol} {filing.filing_type}: {e}"
            )
            return SECFilingAnalysisResult(
                filing=filing,
    ...
    async def _analyze_with_ai(
        self, filing: SECFiling, pdf_path: Path
    ) -> Optional[SECFilingAnalysisResult]:
        """
        ...
        prompt = ANALYSIS_PROMPTS.get(
            filing.filing_type, ANALYSIS_PROMPTS[FilingType.OTHER]
        )
        ...
        try:
            ...
            if not self.openai_client:
                logger.error(
                    "OpenAI client not initialized. Check if OPENAI_API_KEY is properly set in .env file."
                )
                return None

            logger.info(
                f"Analyzing filing using OpenAI with model: {settings.SEC_ANALYSIS_MODEL}"
            )
    ...
    async def _analyze_with_openai_text(
        self, filing: SECFiling, prompt: str, pdf_path: Path
    ) -> Optional[SECFilingAnalysisResult]:
        """
        ...
        try:
            ...
            try:
                ...
                if len(filing_text) > max_chars:
                    logger.warning(
                        f"Filing text too long ({len(filing_text)} chars), truncating to {max_chars} chars"
                    )
                    filing_text = (
                ...
                return SECFilingAnalysisResult(
                    ...
                    analysis={
                        "ERROR": f"Failed to read filing text: {str(read_error)}"
                    },
                )
            ...
            return SECFilingAnalysisResult(
                filing=filing, summary=summary, analysis=analysis
            )
