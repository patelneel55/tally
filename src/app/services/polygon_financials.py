...
class PolygonFinancialsClient:
        ...
        if not force_refresh and cache_file.exists():
            try:
                ...
                cache_date = datetime.fromisoformat(
                    cached_data.get("fetched_at", "2000-01-01")
                )
    ...
    def get_financial_statements_by_type(
        ...
        for filing in annual_data.get("results", []):
            ...
            if (
                statement_type in ["cash", "all"]
                and "cash_flow_statement" in financials
            ):
                cash_flow = {
                    "ticker": ticker,
                    "fiscal_year": fiscal_year,
        ...
        if quarterly_data:
            for filing in quarterly_data.get("results", []):
                ...
                # Extract income statement
                if (
                    statement_type in ["income", "all"]
                    and "income_statement" in financials
                ):
                    income_statement = {
                        "ticker": ticker,
                        "fiscal_year": fiscal_year,
                ...
                # Extract balance sheet
                if (
                    statement_type in ["balance", "all"]
                    and "balance_sheet" in financials
                ):
                    balance_sheet = {
                        "ticker": ticker,
                        "fiscal_year": fiscal_year,
    ...
    def get_common_financial_metrics(
        ...
        result = {"income": {}, "balance": {}, "cash_flow": {}, "ratios": {}}
    ...
    def _apply_rate_limiting(self):
        ...
        if len(self.request_timestamps) >= self.rate_limit_per_minute:
            ...
            if sleep_time > 0:
                logger.info(
                    f"Rate limiting in effect, waiting {sleep_time:.2f} seconds"
                )
