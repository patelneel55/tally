...
class AIAnalyst:
        ...
        if not force_refresh and cache_file.exists():
            try:
                ...
                cache_date = datetime.fromisoformat(
                    cached_data.get("generated_at", "2000-01-01")
                )
        ...
        try:
            ...
            for peer in peers:
                try:
                    ...
                    peer_financial_data = (
                        financial_data_aggregator.get_comprehensive_financial_data(
                            ticker=peer,
                    ...
                    if peer_financial_data:
                        ...
                        peer_data[peer] = self._extract_peer_metrics(
                            peer_financial_data
                        )
                except Exception as e:
                    logger.warning(f"Error gathering data for peer {peer}: {e}")

            return {"peers": list(peer_data.keys()), "peer_data": peer_data}
    ...
    def _extract_peer_metrics(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                if (
                    "revenues" in annual_metrics
                    and len(annual_metrics["revenues"]) >= 2
                ):
    ...
    def _generate_analysis_with_ai(
        self, ticker: str, financial_model: Dict[str, Any], peer_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            prompt = INVESTMENT_ANALYSIS_PROMPT.format(
                ticker=ticker, financial_data_and_model_summary=data_summary
            )
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a senior investment analyst at a top-tier investment bank.",
                    },
    ...
    def _create_data_summary(
        ...
        try:
            ...
            financial_data = financial_data_aggregator.get_comprehensive_financial_data(
                ticker=ticker, force_refresh=False
            )
            ...
            if (
                financial_data
                and "time_series" in financial_data
                and "annual" in financial_data["time_series"]
            ):
                ...
                for year in ["2024", "2023", "2022"]:
                    ...
                    if (
                        ...
                        or year not in net_income_values
                        or revenue_values[year] == 0
                    ):
                        continue

                    net_margin = net_income_values[year] / revenue_values[year]
                    operating_margin = (
                        operating_income_values.get(year, 0) / revenue_values[year]
                        if year in operating_income_values
                        else 0
                    )
                    roa = (
                        net_income_values[year] / total_assets_values.get(year, 0)
                        if year in total_assets_values
                        and total_assets_values[year] != 0
                        else 0
                    )
                    roe = (
                        net_income_values[year] / total_equity_values.get(year, 0)
                        if year in total_equity_values
                        and total_equity_values[year] != 0
                        else 0
                    )

                    # Add ratio rows
                    if "Net Margin" not in [
                        row.split("|")[1].strip() for row in ratio_table
                    ]:
                        row = "| Net Margin | "
                ...
                if years:
                    ...
                    for metric_key, metric_name in income_metrics:
                        ...
                        for year in years:
                            if (
                                year in annual_data
                                and "income_statement" in annual_data[year]
                            ):
                                income_stmt = annual_data[year]["income_statement"]
                                if (
                                    metric_key in income_stmt
                                    and "value" in income_stmt[metric_key]
                                ):
                    ...
                    for metric_key, metric_name in balance_metrics:
                        ...
                        for year in years:
                            if (
                                year in annual_data
                                and "balance_sheet" in annual_data[year]
                            ):
                                balance_sheet = annual_data[year]["balance_sheet"]
                                if (
                                    metric_key in balance_sheet
                                    and "value" in balance_sheet[metric_key]
                                ):
                    ...
                    for metric_key, metric_name in cash_flow_metrics:
                        ...
                        for year in years:
                            if (
                                year in annual_data
                                and "cash_flow_statement" in annual_data[year]
                            ):
                                cash_flow = annual_data[year]["cash_flow_statement"]
                                if (
                                    metric_key in cash_flow
                                    and "value" in cash_flow[metric_key]
                                ):
                    ...
                    for year in years:
                        if year in annual_data:
                            ...
                            net_income = income_stmt.get("net_income_loss", {}).get(
                                "value"
                            )
                            operating_income = income_stmt.get(
                        ...
                        if values:
                            ...
                            for i, item in enumerate(values):
                                if (
                                    ...
                                    if prev_value and prev_value != 0:
                                        qoq_change = (
                                            f"{(value - prev_value) / prev_value:.2%}"
                                        )
                                    ...
                                    if (
                                        len(values) > 4
                                        and i + 4 < len(values)
                                        and "value" in values[i + 4]
                                    ):
                                        yoy_comp = values[i + 4]["value"]
                                        if yoy_comp and yoy_comp != 0:
                                            yoy_change = (
                                                f"{(value - yoy_comp) / yoy_comp:.2%}"
                                            )

                                    ts_table.append(
                                        f"| Q{quarter} {year} | {value:,.2f} | {qoq_change} | {yoy_change} |"
                                    )
            ...
            if "historical_analysis" in financial_model:
                ...
                if "profitability_metrics" in historical:
                    ...
                    summary.append(
                        json.dumps(historical["profitability_metrics"], indent=2)
                    )
                ...
                if "balance_sheet_metrics" in historical:
                    ...
                    summary.append(
                        json.dumps(historical["balance_sheet_metrics"], indent=2)
                    )
                ...
                if peers and peer_metrics:
                    ...
                    if financial_data:
                        ...
                        if (
                            ...
                            if (
                                "revenues" in annual_metrics
                                and annual_metrics["revenues"]
                            ):
                                company_metrics["revenue"] = annual_metrics["revenues"][
                                    0
                                ]["value"]

                            if (
                                "operating_income" in annual_metrics
                                and annual_metrics["operating_income"]
                            ):
                                company_metrics["operating_income"] = annual_metrics[
                                    "operating_income"
                                ][0]["value"]

                            if (
                                "net_income" in annual_metrics
                                and annual_metrics["net_income"]
                            ):
                                company_metrics["net_income"] = annual_metrics[
                                    "net_income"
                                ][0]["value"]

                            if (
                                "total_assets" in annual_metrics
                                and annual_metrics["total_assets"]
                            ):
                                company_metrics["total_assets"] = annual_metrics[
                                    "total_assets"
                                ][0]["value"]

                            if (
                                "total_liabilities" in annual_metrics
                                and annual_metrics["total_liabilities"]
                            ):
                                company_metrics["total_liabilities"] = annual_metrics[
                                    "total_liabilities"
                                ][0]["value"]

                            if (
                                "total_equity" in annual_metrics
                                and annual_metrics["total_equity"]
                            ):
                                company_metrics["total_equity"] = annual_metrics[
                                    "total_equity"
                                ][0]["value"]
                        ...
                        # Calculate ratios
                        if (
                            "revenue" in company_metrics
                            and "net_income" in company_metrics
                            and company_metrics["revenue"]
                        ):
                            company_metrics["net_margin"] = (
                                company_metrics["net_income"]
                                / company_metrics["revenue"]
                            )

                        if (
                            "total_assets" in company_metrics
                            and "net_income" in company_metrics
                            and company_metrics["total_assets"]
                        ):
                            company_metrics["roa"] = (
                                company_metrics["net_income"]
                                / company_metrics["total_assets"]
                            )

                        if (
                            "total_equity" in company_metrics
                            and "net_income" in company_metrics
                            and company_metrics["total_equity"]
                        ):
                            company_metrics["roe"] = (
                                company_metrics["net_income"]
                                / company_metrics["total_equity"]
                            )
    ...
    def _process_ai_analysis_response(self, analysis_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "Financial Performance Analysis" in sections:
                analysis["financial_performance"] = sections[
            ...
            if "Valuation Analysis" in sections:
                ...
                price_target = self._extract_price_target(
                    sections["Valuation Analysis"]
                )
                if price_target:
    ...
    def _extract_price_target(self, valuation_text: str) -> Dict[str, Any]:
        ...
        if "value" in price_target:
            ...
            downside_matches = re.findall(
                downside_pattern, valuation_text, re.IGNORECASE
            )
            if downside_matches:
    ...
    def _generate_executive_summary(self, ticker: str, analysis: Dict[str, Any]) -> str:
        ...
        try:
            ...
            prompt = EXECUTIVE_SUMMARY_PROMPT.format(
                ticker=ticker, detailed_analysis=detailed_analysis
            )
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are the Chief Investment Officer at a major asset management firm.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
    ...
    def _generate_competitive_analysis(
        self, ticker: str, analysis: Dict[str, Any], peer_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            financial_and_competitor_data = json.dumps(
                {"company_analysis": analysis, "peer_data": peer_data}, indent=2
            )
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a strategic management consultant specializing in competitive analysis.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
