...
class FinancialStatementExtractor:
    ...
    def extract_financial_statements(
        ...
        """
        logger.info(
            f"Extracting financial statements from {filing.filing_type} for {filing.ticker}"
        )
        ...
        if cached_result:
            logger.info(
                f"Retrieved financial statements from cache for {filing.ticker}"
            )
        ...
        if not content:
            logger.warning(
                f"No content found for {filing.filing_type} filing {filing.ticker}"
            )
            return {}
        ...
        for statement_type in FinancialStatementType:
            try:
                ...
                logger.error(
                    f"Error extracting {statement_type} for {filing.ticker}: {str(e)}"
                )
    ...
    def _extract_statement(
        ...
        if not section:
            logger.warning(
                f"Could not find {statement_type} section in {filing.ticker} filing"
            )
            return None
        ...
        try:
            ...
            if not data or not data.get("metrics"):
                logger.warning(
                    f"Failed to extract metrics from {statement_type} for {filing.ticker}"
                )
                return None
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                for end_pattern in end_markers:
                    end_match = re.search(
                        end_pattern, content[start_idx : start_idx + 50000]
                    )
                    if end_match:
    ...
    def _parse_html_table(
        ...
        for row in target_table.find_all("tr")[1:]:  # Skip header row
            ...
            standardized_name = self._standardize_metric_name(
        ...
        return {"metrics": metrics, "units": self._determine_units(section)}
    ...
    def _parse_text_table(
        ...
        for i, line in enumerate(lines[:20]):  # Check first 20 lines for header
            if re.search(r"\b20\d\d\b", line) and (
                "year" in line.lower()
                or "period" in line.lower()
                or len(re.findall(r"\b20\d\d\b", line)) >= 2
            ):
                header_line = i
                break
    ...
    def _determine_period(
        self, filing: SECFiling, data: Dict[str, Any]
    ) -> FinancialStatementPeriod:
        """
        ...
        return (
            ...
            if has_quarterly
            else FinancialStatementPeriod.ANNUAL
        )
    ...
    def _get_from_cache(
        self, cache_key: str
    ) -> Optional[Dict[str, FinancialStatement]]:
        """
