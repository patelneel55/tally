...
from app.services.polygon_financials import polygon_financials
...
class FinancialDataAggregator:
    ...
    def get_comprehensive_financial_data(
        ...
        if not force_refresh and cache_file.exists():
            try:
                ...
                cache_date = datetime.fromisoformat(
                    cached_data.get("aggregated_at", "2000-01-01")
                )
        ...
        try:
            ...
            if not financial_statements or not any(financial_statements.values()):
                logger.warning(
                    f"No financial statements found for {ticker} from Polygon.io"
                )
            ...
            try:
                ...
                for year in range(current_year, current_year - years, -1):
                    try:
                        ...
                        logger.warning(
                            f"Error fetching 10-K for {ticker} for year {year}: {e}"
                        )
            except Exception as e:
            ...
            for year, filing in filings:
                try:
                    ...
                    if analysis_result:
                        ...
                        accounting_policies[year] = self._extract_accounting_policies(
                            filing, analysis_result
                        )
                        footnotes[year] = self._extract_footnotes(
                            filing, analysis_result
                        )
                except Exception as e:
                    logger.warning(
                        f"Error extracting policies for {ticker} for year {year}: {e}"
                    )
        except Exception as e:
        ...
        quarterly_data = self._organize_statements_by_year(
    ...
    def _get_latest_10k(self, ticker: str, year: int) -> Optional[SECFiling]:
        ...
        try:
            ...
            try:
                ...
                if (
                    filings
                    and FilingType.FORM_10K in filings
                    and len(filings[FilingType.FORM_10K]) > 0
                ):
            ...
            try:
                ...
                filing = sec_filing_fetcher.get_10k(ticker=ticker, year=year)
    ...
    def _extract_accounting_policies(
        self, filing: SECFiling, analysis_result: Any
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(analysis_result, "significant_accounting_policies"):
                accounting_policies["significant_policies"] = (
                    analysis_result.significant_accounting_policies
                )
            ...
            for policy_name in [
                ...
                if hasattr(analysis_result, f"{policy_name}_policy"):
                    accounting_policies[policy_name] = getattr(
                        analysis_result, f"{policy_name}_policy"
                    )
        except Exception as e:
    ...
    def _organize_statements_by_year(
        self, statements: Dict[str, List[Dict[str, Any]]], period_type: str
    ...
    def _create_time_series(
        ...
        for metric_name in time_series["quarterly"]:
            time_series["quarterly"][metric_name].sort(
                key=lambda x: (x.get("year", ""), x.get("quarter", ""))
            )
