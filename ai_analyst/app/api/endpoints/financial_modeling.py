...
from ai_analyst.app.services.ai_financial_modeler import ai_financial_modeler
...
@router.post("/model", response_model=ModelingResponse, summary="Generate financial model")
async def generate_financial_model(request: ModelingRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate financial model: {str(e)}")

@router.post("/model/async", response_model=ModelJobResponse, summary="Generate financial model asynchronously")
async def generate_financial_model_async(
    ...
    try:
        ...
        modeling_jobs[job_id] = {
            ...
            "estimated_completion_time": datetime.now().timestamp() + 300,  # Estimate 5 minutes
            "result": None
        }
        ...
        return ModelJobResponse(
            ...
            ticker=request.ticker,
            status="pending",
            estimated_completion_time=datetime.fromtimestamp(modeling_jobs[job_id]["estimated_completion_time"])
        )
        
    except Exception as e:
        logger.error(f"Error starting modeling job: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to start modeling job: {str(e)}")
...
async def get_modeling_job_status(job_id: str):
    ...
    try:
        ...
        if job_id not in modeling_jobs:
            raise HTTPException(status_code=404, detail=f"Modeling job {job_id} not found")
        ...
        return {
            ...
            "estimated_completion_time": datetime.fromtimestamp(job["estimated_completion_time"])
        }
...
async def get_quick_model(
    symbol: str,
    years: int = Query(3, description="Years of historical data")
):
    """
    ...
    try:
        ...
        if "time_series" in financial_data and "annual" in financial_data["time_series"]:
            ...
            if "Revenue" in time_series:
                ...
                if len(revenue_data) >= 2:
                    ...
                    if growth_rates:
                        avg_growth_rate = sum(g["growth_rate"] for g in growth_rates) / len(growth_rates)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate quick model: {str(e)}") 