...
SEC_QUERY_API_ENDPOINT = "https://api.sec-api.io"  # Base endpoint for all SEC API requests
...
PREMIUM_FILING_DOWNLOAD_LIMIT = 7000 / 300  # Filings per second (7000 per 5 minutes = ~23.3 per second)
...
class SECFilingFetcher:
    ...
    def __init__(self):
        ...
        self.min_query_interval = 1.0 / PREMIUM_RATE_LIMIT  # e.g., 1/20 = 0.05s between requests
        self.min_search_interval = 1.0 / PREMIUM_SEARCH_RATE_LIMIT  # e.g., 1/10 = 0.1s between requests
        self.min_download_interval = 1.0 / PREMIUM_FILING_DOWNLOAD_LIMIT  # Time between download requests
    ...
    async def get_filing_pdf(self, filing: SECFiling) -> Optional[Path]:
        ...
        if cache_path.exists() and settings.ENABLE_CACHE:
            logger.info(f"Using cached PDF for {filing.symbol} {filing.filing_type} from {filing.filing_date}")
            return cache_path
        ...
        logger.info(f"Downloading {filing.filing_type} filing for {filing.symbol} from {filing.filing_date} as PDF")
        ...
        try:
            ...
            async with self.query_semaphore:
                ...
                if pdf_data:
                    ...
                    logger.info(f"Successfully downloaded and cached PDF for {filing.symbol} {filing.filing_type}")
                    return cache_path
                else:
                    logger.error(f"Failed to download PDF for {filing.symbol} {filing.filing_type}")
                    return None
                    
        except Exception as e:
            logger.error(f"Error downloading PDF for {filing.symbol} {filing.filing_type}: {e}")
            return None
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status == 200:
                        ...
                        elif response.status == 202:  # Cache miss, PDF generation started
                            ...
                            if "message" in response_json:
                                ...
                                retry_match = re.search(r"Retry in (\d+) seconds", message)
                                if retry_match:
                                    retry_seconds = int(retry_match.group(1))
                            
                            # Wait for the specified time before retrying
                            logger.info(f"Waiting {retry_seconds} seconds for PDF generation to complete")
    ...
    async def batch_download_filings(self, filings: List[SECFiling]) -> Dict[str, Optional[Path]]:
        """
        ...
        async def download_with_rate_limit(filing: SECFiling) -> Tuple[str, Optional[Path]]:
            ...
            async with batch_semaphore:
                ...
                if time_since_last_request < self.min_download_interval:
                    await asyncio.sleep(self.min_download_interval - time_since_last_request)
                ...
                try:
                    ...
                    return filing.filing_id, pdf_path
                except Exception as e:
                    logger.error(f"Error downloading {filing.symbol} {filing.filing_type}: {e}")
                    return filing.filing_id, None
    ...
    async def get_filing_by_accession_number(self, accession_number: str, symbol: str) -> Optional[Path]:
        """
    ...
    async def get_historical_filings(self, symbol: str) -> Dict[str, List[SECFiling]]:
        ...
        try:
            ...
            form_10k_filings = await self._query_sec_filings(
                symbol=symbol,
                form_type="10-K",
                limit=HISTORICAL_10K_COUNT
            )
            
            form_10q_filings = await self._query_sec_filings(
                symbol=symbol,
                form_type="10-Q",
                limit=HISTORICAL_10Q_COUNT
            )
            
            # Step 2: Convert API results to SECFiling objects
            for filing_data in form_10k_filings:
                filing = self._convert_api_result_to_filing(filing_data, symbol, FilingType.FORM_10K)
                if filing:
                    historical_filings["10-K"].append(filing)
            
            for filing_data in form_10q_filings:
                filing = self._convert_api_result_to_filing(filing_data, symbol, FilingType.FORM_10Q)
                if filing:
            ...
            if all_filings:
                logger.info(f"Downloading {len(all_filings)} historical filings for {symbol}")
    ...
    async def _query_sec_filings(self, symbol: str, form_type: str = None, limit: int = 5) -> List[Dict]:
        """
        ...
        async with self.query_semaphore:
            ...
            try:
                ...
                logger.info(f"API Endpoint: {SEC_QUERY_API_ENDPOINT}/query")
                logger.info(f"Querying SEC API for {symbol} filings with payload: {json.dumps(query_payload)}")
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        f"{SEC_QUERY_API_ENDPOINT}/query",  # Add the /query path to the base endpoint
                        headers=headers,
                        json=query_payload
                    ) as response:
                        ...
                        if response.status == 200:
                            ...
                            logger.error(f"SEC API error: {response.status}, {response_text}")
    ...
    def _convert_api_result_to_filing(self, filing_data: Dict, symbol: str, filing_type: FilingType) -> Optional[SECFiling]:
        """
...
class MockSECFilingFetcher:
    ...
    def get_filing(self, ticker: str, filing_type: FilingType, fiscal_year: int, fiscal_period: str = None):
        """
        ...
        """
        self.logger.info(f"Mock fetcher returning filing for {ticker} {filing_type} {fiscal_year} {fiscal_period}")
        
        # Create a mock filing object
        filing_date = datetime(fiscal_year, 12, 31) if filing_type == FilingType.FORM_10K else datetime(fiscal_year, 3, 31)
