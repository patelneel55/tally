...
from ai_analyst.app.services.sec_analyzer import sec_filing_analyzer, SECFilingAnalysisResult
...
class SECTrendsAnalysisResult:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], filings: Dict[str, List[SECFiling]]) -> 'SECTrendsAnalysisResult':
...
class SECTrendsAnalyzer:
    ...
    async def analyze_historical_filings(self, symbol: str) -> Optional[SECTrendsAnalysisResult]:
        """
        ...
        if cache_path.exists() and settings.ENABLE_CACHE:
            ...
            try:
                ...
                return SECTrendsAnalysisResult.from_dict(analysis_data, historical_filings)
        ...
        try:
            ...
            all_filings = historical_filings.get("10-K", []) + historical_filings.get("10-Q", [])
            if not all_filings:
    ...
    async def _analyze_trends(
        self, 
        symbol: str, 
        filing_analyses: Dict[str, List[SECFilingAnalysisResult]]
    ...
    def _prepare_trends_input(self, symbol: str, filing_analyses: Dict[str, List[SECFilingAnalysisResult]]) -> str:
        """
        ...
        if "10-K" in filing_analyses and filing_analyses["10-K"]:
            ...
            for analysis in sorted(filing_analyses["10-K"], key=lambda x: x.filing.filing_date, reverse=True):
                input_parts.append(f"### 10-K Filing Date: {analysis.filing.filing_date.isoformat()}\n\n")
        ...
        if "10-Q" in filing_analyses and filing_analyses["10-Q"]:
            ...
            for analysis in sorted(filing_analyses["10-Q"], key=lambda x: x.filing.filing_date, reverse=True):
                input_parts.append(f"### 10-Q Filing Date: {analysis.filing.filing_date.isoformat()}\n\n")
                input_parts.append(f"Summary: {analysis.summary}\n\n")
                for section, content in analysis.analysis.items():
                    input_parts.append(f"#### {section}\n{content}\n\n")
