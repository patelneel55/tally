...
class TestSECFetcher(unittest.TestCase):
    ...
    def test_build_search_query(self):
        ...
        # Test with ticker
        request = FilingRequest(
            identifier="AAPL", filing_type=FilingType.QUARTERLY_REPORT
        )
        query = self.fetcher._build_search_query(request)
    ...
    def test_convert_api_results_to_filings(self):
        ...
        self.assertEqual(
            ...
            "https://www.sec.gov/Archives/edgar/data/924383/000143774920002201/lrad20191231_10q.htm",
        )
    ...
    async def test_download_filing_as_pdf(self, mock_request):
        ...
        # Download PDF
        pdf_data = await self.fetcher._download_filing_as_pdf(
            "https://www.sec.gov/test.htm"
        )
    ...
    async def test_fetch_pdf(self, mock_process, mock_fetch_api):
        ...
        filings = await self.fetcher.fetch(
            "TEST", filing_type=FilingType.ANNUAL_REPORT, data_format=DataFormat.PDF
        )
    ...
    async def test_fetch_html(self, mock_process, mock_fetch_api):
        ...
        # Fetch filings
        filings = await self.fetcher.fetch(
            "TEST", filing_type=FilingType.ANNUAL_REPORT, data_format=DataFormat.HTML
        )
