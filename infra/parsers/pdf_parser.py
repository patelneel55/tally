...
class PDFParser(IParser):
    ...
    def __init__(
        self, chunk_size: Optional[int] = 1000, chunk_overlap: Optional[int] = 200
    ):
        pass

    def parse(
        self, file_path: str, output_format: IParser.SUPPORTED_FORMATS = "markdown"
    ) -> List[Document]:
        """
    ...
    def write_file(self, documents: List[Document], output_path: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully wrote {len(documents)} documents to {output_path}"
            )
        except Exception as e:
