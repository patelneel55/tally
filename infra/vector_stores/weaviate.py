...
class WeaviateVectorStore(IVectorStore):
    ...
    def _ensure_class_exists(self, collection_name: str, metadata: Dict):
        ...
        existing_classes = list(self._get_client().collections.list_all(simple=True).keys())
        class_exists = collection_name in existing_classes
        properties = {
            k: Property(name=k, data_type=self._get_weaviate_type(type(v)), skip_vectorization=True)
            for k, v in metadata.items()
        }
        properties[self._text_key] = Property(name=self._text_key, data_type=DataType.TEXT)
        ...
        if class_exists:
            ...
            existing_props = {prop.name for prop in collection.config.get(simple=True).properties}
