...
from infra.ratelimit.models import RateLimitRule, GlobalRateLimiterConfig, AlgorithmType
...
class RateLimiter:
            ...
            for rule_name, rule_config in self.rules.items():
                if rule_name not in self.adapters:
                    ...
                    if rule_config.algorithm == AlgorithmType.LEAKY_BUCKET:
                        ...
                        raise RateLimiterConfigError(f"Unsupported async algorithm_type: {rule_config.algorithm_type} for rule '{rule_name}'")
    ...
    async def acquire(self, rules: List[Tuple[str, int]], identifier: str = "global"):
        for rule_name, cost in rules:
            if rule_name not in self.rules:
                raise RateLimiterConfigError(f"Rule '{rule_name}' in multi-acquire not defined.")
            if cost <= 0:
                raise ValueError(f"Cost for rule '{rule_name}' in multi-acquire must be positive ({cost} given).")

            adapter: BaseRateLimiterAlgorithm = await self._get_or_create_adapter(rule_name)
            await adapter.consume(self.rules[rule_name], identifier=identifier, cost=cost)
